...
export default function Experience({vertCount}){
    
    //useThree gives us acces to camera, scene, renderer, clock . . . .
    const { camera, gl } = useThree()   //destructure and retrieve camera and renderer (gl)
    ...
    useFrame((state,delta)=>{
        ...
        state.camera.position.x = Math.cos(state.clock.elapsedTime/2 )  * 10
        state.camera.position.z = Math.sin(state.clock.elapsedTime /2 ) * 10
    ...
    return (
        <>
            ...
            <group ref={groupRef} >
                ...
                <CustomObject vertCount={vertCount}/>
